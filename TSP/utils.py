...
main_dir = "" # "TSP_cluster/"
process_start_time = datetime.now()
result_folder = f'./{main_dir}result/' + process_start_time.strftime("%Y%m%d_%H%M%S") + '{desc}'
...
class TimeEstimator:
    ...
    def get_est_string(self, count, total):
        ...
        elapsed_time_str = get_time_str(elapsed_time)
        remain_time_str = get_time_str(remain_time)
        return elapsed_time_str, remain_time_str

    def get_total_time(self, n_instances):
        elapsed_time, _ = self.get_est(1, 1)
        elapsed_per_instance = elapsed_time / n_instances
        elapsed_time_str = get_time_str(elapsed_time)
        elapsed_time_per_instance_str = get_time_str(elapsed_per_instance)

        return elapsed_time_str, elapsed_time_per_instance_str
...
def get_time_str(hours):
    if hours > 1.0:
        time_str = "{:.2f}h".format(hours)
    else:
        hours *= 60
        if hours > 1.0:
            time_str = "{:.2f}m".format(hours)
        else:
            hours *= 60
            time_str = "{:.2f}s".format(hours)
