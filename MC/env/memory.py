...
def one_hot_encoding(state, action):
    ...
    double_action[cur_values == 1] += state_dim

    # Perform one-hot encoding
    one_hot_actions = torch.nn.functional.one_hot(double_action, num_classes=2 * state_dim).float()
    one_hot_actions = one_hot_actions.view(batch_size, 2, state_dim)
    one_hot_actions = one_hot_actions.transpose(1, 2).contiguous().view(batch_size, state_dim, 2)
    return one_hot_actions


class NewMemory:
    def __init__(self, state_dim, memory_aggr, memory_size=20000, n_memories=1, device='cpu'):
        """
        ...
        max_mem = memory_size # 0
        self.state_memories = [torch.zeros((max_mem, self.state_dim)) for _ in range(self.n_memories)]
        self.action_memories = [torch.zeros((max_mem, self.state_dim, 2)) for _ in range(self.n_memories)]

        self.used_memory = 0

    def save_in_memory(self, state, action):
        """
        state.shape = (batch_size, state_dim). Solutions are encoded as 0/1
        action.shape = (batch_size). Actions are encoded as 0/1
        need to one hot encode the actions in 2*state_dim dimensions, based on the value of the state
        """
        batch_size = state.shape[0]

        one_hot_actions = one_hot_encoding(state, action)
        ...
        for idx in range(self.n_memories):
            ...
            if self.used_memory >= self.memory_size:
                ...
                if self.n_memories == 1:
                    self.state_memories[idx][self.used_memory:self.used_memory+batch_size] = state
                    self.action_memories[idx][self.used_memory:self.used_memory+batch_size] = one_hot_actions
                else:
                    self.state_memories[idx][self.used_memory] = state[idx]
                    self.action_memories[idx][self.used_memory] = one_hot_actions[idx]
    ...
    def get_knn(self, state, k):
        ...
        for idx in range(self.n_memories):
            if self.n_memories != 1:  # Multiple memories
                ...
                inner_products = torch.mm(state[idx:idx+1], self.state_memories[idx][:self.used_memory].t())  # Result is pomo_size * used_memory
                ...
                nearest_acts = self.action_memories[idx][:self.used_memory][indices.flatten(), :].reshape(indices.shape + (self.state_dim, 2))
                ...
                inner_products = torch.mm(state, self.state_memories[idx][:self.used_memory].t())  # Result is pomo_size * used_memory
                similarity, indices = torch.topk(inner_products, k, largest=True, sorted=True)
                ...
                nearest_acts = self.action_memories[idx][:self.used_memory][indices, :].reshape(indices.shape + (self.state_dim, 2))
...
class Memory:
    def __init__(self, state_dim, memory_aggr, memory_size=100000, n_memories=1, device='cpu'):
        """
        Memory that saves State-Action pairs and returns the average value of the k-nearest neighbours of a state
        Key: Solution state of the problem
        Value: One-hot encoding of the performed action
